...
class ProdutoDao:
    ...
    def inserir(produto: Produto) -> int:
        sql = "INSERT INTO t_produto (nome, preco, estoque) VALUES (%s, %s, %s)"
        with Conexao().abrir() as cx:
            cx.cur.execute(sql, (produto.nome, produto.preco, produto.estoque))
    ...
    def atualizar(produto: Produto) -> bool:
        sql = "UPDATE t_produto SET nome=%s, preco=%s, estoque=%s WHERE id_produto=%s"
        with Conexao().abrir() as cx:
            cx.cur.execute(sql, (produto.nome, produto.preco, produto.estoque, produto.id_produto))
